...
public class ExportCsvTest {
    ...
    public static void setUp() throws Exception {
        ...
        hdfsUrl = String.format("%s/user/%s/all.csv", miniDFSCluster.getURI().toString(), System.getProperty("user.name"));
    }
    ...
    public void testExportAllCsvHDFS() throws Exception {
        final AtomicReference<FSDataInputStream> inputStream = new AtomicReference<>();
        TestUtil.testCall(db, "CALL apoc.export.csv.all($file,null)", map("file", hdfsUrl),
                (r) -> {
                    try {
                        FileSystem fs = miniDFSCluster.getFileSystem();
                        inputStream.set(fs.open(new Path(String.format("/user/%s/all.csv", System.getProperty("user.name")))));
                        File output = Files.createTempFile("all", ".csv").toFile();
                        FileUtils.copyInputStreamToFile(inputStream.get(), output);
        ...
        // append mode
        TestUtil.testCall(db, "CALL apoc.export.csv.all($file,{append:false})", map("file", hdfsUrl),
                (r) -> {
                    try {
                        File output = Files.createTempFile("all", ".csv").toFile();
                        FileUtils.copyInputStreamToFile(inputStream.get(), output);
                        ...
                        assertEquals("csv", r.get("format"));
                        assertTrue("Should get time greater than 0",((long) r.get("time")) >= 0);
                        assertEquals(EXPECTED_HEADER + EXPECTED_CONTENT + EXPECTED_CONTENT, TestUtil.readFileToString(output));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                });
    ...
    @Test
    public void testExportHDFSFailsWithAppendNonExistentFile() {
        try {
            TestUtil.testCall(db, "CALL apoc.export.csv.all($file,{append: true})", map("file", hdfsUrl),
                    r -> Assert.fail("Non-existent file"));
        } catch (RuntimeException e) {
            // handled through org.apache.hadoop.hdfs.server.namenode.FSDirAppendOp
            assertTrue(e.getMessage().contains("Failed to append to non-existent file"));
        }
    }
