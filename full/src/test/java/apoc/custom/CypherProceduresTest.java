...
public class CypherProceduresTest  {
    ...
    public void shouldDeclareFunctionWithDefaultListAndMaps() {
        db.executeTransactionally("CALL apoc.custom.declareFunction('funWithFloatList(minScore = [1.1,2.2,3.3] :: LIST OF FLOAT) :: BOOLEAN',\n" +
                "    'return size($minScore) < 4 as res')");
        testCall(db, "RETURN custom.procWithFloatList() AS res", (row) -> {
            assertEquals(true, row.get("res"));
        });
        testCall(db, "RETURN custom.procWithFloatList([9.1, 2.6, 3.1, 4.3, 5.5]) AS res", (row) -> {
            assertEquals(false, row.get("res"));
        });

        db.executeTransactionally("CALL apoc.custom.declareFunction('funWithIntList(minScore = [1,2,3] :: LIST OF INT) :: BOOLEAN',\n" +
                "    'return size($minScore) < 4 as res')");
        testCall(db, "RETURN custom.procWithIntList() AS res", (row) -> {
            assertEquals(true, row.get("res"));
        });
        testCall(db, "RETURN custom.procWithIntList([9,2,3,4,5]) AS res", (row) -> {
            assertEquals(false, row.get("res"));
        });

        db.executeTransactionally("CALL apoc.custom.declareFunction('funWithListString(minScore = [\"1\",\"2\",\"3\"] :: LIST OF STRING) :: BOOLEAN',\n" +
                "    'return size($minScore) < 4 as res')");
        testCall(db, "RETURN custom.procWithListString() AS res", (row) -> {
            assertEquals(true, row.get("res"));
        });
        testCall(db, "RETURN custom.procWithListString(['aaa','bbb','ccc','ddd','eee']) AS res", (row) -> {
            assertEquals(false, row.get("res"));
        });

        db.executeTransactionally("CALL apoc.custom.declareFunction('funWithListStringPlain(minScore = [1, 2, 3] :: LIST OF STRING) :: BOOLEAN',\n" +
                "    'return size($minScore) < 4 as res')");
        testCall(db, "RETURN custom.procWithListStringPlain() AS res", (row) -> {
            assertEquals(true, row.get("res"));
        });
        testCall(db, "RETURN custom.procWithListStringPlain(['aaa','bbb','ccc','ddd','eee']) AS res", (row) -> {
            assertEquals(false, row.get("res"));
        });
        
        db.executeTransactionally("CALL apoc.custom.declareFunction(\"procWithListStringQuoted(minScore = ['1','2','3'] :: LIST OF STRING) :: BOOLEAN\",\n" +
                "    'return size($minScore) < 4 as res')");
        testCall(db, "RETURN custom.procWithListStringQuoted() AS res", (row) -> {
            assertEquals(true, row.get("res"));
        });
        testCall(db, "RETURN custom.procWithListStringQuoted(['aaa','bbb','ccc','ddd','eee']) AS res", (row) -> {
            assertEquals(false, row.get("res"));
        });
        ...
        db.executeTransactionally("CALL apoc.custom.declareFunction('funWithListStringVars(minScore = [true,false,null] :: LIST OF STRING) :: BOOLEAN',\n" +
                "    'return size($minScore) < 4 as res')");
        testCall(db, "RETURN custom.procWithListStringVars() AS res", (row) -> {
            assertEquals(true, row.get("res"));
        });
        testCall(db, "RETURN custom.procWithListStringVars(['aaa','bbb','ccc','ddd','eee']) AS res", (row) -> {
            assertEquals(false, row.get("res"));
        });
        ...
        testCall(db, "RETURN custom.procWithMapList() AS res", (row) -> {
            assertEquals(Map.of("res", Map.of("aa", 1L, "bb", "2")), row.get("res"));
        });
        testCall(db, "RETURN custom.procWithMapList({c: true}) AS res", (row) -> {
    ...
    public void shouldDeclareProcedureWithDefaultString() {
        ...
        testCall(db, "CALL custom.procWithNullString('myText')", (row) -> assertEquals("myText - suffix", row.get("res")));
    ...
    public void shouldDeclareFunctionWithDefaultString() {
        ...
        testCall(db, "RETURN custom.procWithNullString('myText') AS res", (row) -> assertEquals("myText - suffix", row.get("res")));
        ...
        db.executeTransactionally("CALL apoc.custom.declareFunction('procWithStringNull(minScore = null :: STRING) :: STRING', $query)",
                Map.of("query", query));
        testCall(db, "RETURN custom.procWithStringNull() AS res", (row) -> assertNull(row.get("res")));
        testCall(db, "RETURN custom.procWithStringNull('other') AS res", (row) -> assertEquals("other - suffix", row.get("res")));
    }
