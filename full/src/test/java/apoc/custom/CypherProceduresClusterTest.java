...
import org.neo4j.junit.jupiter.causal_cluster.ReadReplicaModifier;
import org.testcontainers.containers.Neo4jContainer;

import java.util.List;
import java.util.Map;

import static apoc.util.TestContainerUtil.DOCKER_ENTERPRISE_IMAGE;
import static apoc.util.TestContainerUtil.PASSWORD;
import static apoc.util.TestContainerUtil.TIMEOUT;
import static apoc.util.TestContainerUtil.initializeEnterpriseDb;
...
@NeedsCausalCluster(password = PASSWORD, startupTimeOutInMillis = TIMEOUT, customImageName = DOCKER_ENTERPRISE_IMAGE,
        numberOfCoreServers = 3, numberOfReadReplicas = 1)
public class CypherProceduresClusterTest {
    ...
    public static void setupCluster() {
        ...
        assumeNotNull(URI);
        cluster = new TestcontainersCausalCluster(URI, "neo4j", PASSWORD);
    }
    ...
    @CoreModifier
    @ReadReplicaModifier
    static Neo4jContainer<?> instanceModifier(Neo4jContainer<?> instance) {
        initializeEnterpriseDb(instance);
        return instance.withEnv("apoc.custom.procedures.refresh", "100");
    }
    ...
    public void shouldRegisterSimpleStatementOnOtherClusterMembers() throws InterruptedException {
        ...
        // when
        TestContainerUtil.testCall(cluster.getSession(), "call custom.answerProcedure1()", (row) -> assertEquals(33L, row.get("answer")));
        Thread.sleep(1000);
        // then
        TestContainerUtil.testCallInReadTransaction(cluster.getSession(), "call custom.answerProcedure1()", (row) -> assertEquals(33L, row.get("answer")));
    }
    ...
    public void shouldUpdateSimpleStatementOnOtherClusterMembers() throws InterruptedException {
        ...
        TestContainerUtil.testCall(cluster.getSession(), "call custom.answerProcedure2()", (row) -> assertEquals(33L, row.get("answer")));
        ...
        Thread.sleep(1000);
        // then
        TestContainerUtil.testCallInReadTransaction(cluster.getSession(), "call custom.answerProcedure2()", (row) -> assertEquals(55L, row.get("answer")));
    }
    ...
    public void shouldRemoveProcedureOnOtherClusterMembers() throws InterruptedException {
        assertThrows(DatabaseException.class, () -> {
            ...
            Thread.sleep(1000);
            try {
                TestContainerUtil.testCallInReadTransaction(cluster.getSession(), "call custom.answerToRemove()", (row) -> assertEquals(33L, row.get("answer")));
            } catch (Exception e) {
