...
In case of all procedures, except the `sagemaker.list`, the default `endpoint` is `"https://runtime.sagemaker.<regionConfigValue>.amazonaws.com/endpoints/<endpointNameConfigValue>/invocations"`.
...
The `<endpointNameConfigValue>` part must be configured if we use the `ml.sagemaker.custom` procedure,
...
[source,cypher]
----
CALL apoc.ml.sagemaker.chat([{content: "Only answer with a single word"}
])
----
...
[source,cypher]
----
CALL apoc.ml.sagemaker.completion('What color is the sky? Answer in one word: ')
----
...
[source,cypher]
----
CALL apoc.ml.sagemaker.embedding(['Some Text'], {})
----
...
For example, we can execute the following query (note that the `body` parameter is null, since the API does not have a request body.):

[source,cypher]
----
CALL apoc.ml.sagemaker.custom(null,{
    endpoint: "https://sagemaker.us-east-1.amazonaws.com/<restOfendpoint>",
    method: "GET"
})
----
