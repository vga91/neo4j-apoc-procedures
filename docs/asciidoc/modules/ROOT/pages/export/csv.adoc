...
You can append the procedure result to an existing file putting as a configuration paramenter, `append: true`.
For example

[source,cypher]
----
CALL apoc.export.csv.query("MATCH path = (person:Person)-[:DIRECTED]->(movie)
      RETURN person.name AS name, person.born AS born,
...
Since the result is appended, you have to be sure that 
the header columns and the content formatting (quotes, escape characters, etc... ) match.

The above query has a result like this:
