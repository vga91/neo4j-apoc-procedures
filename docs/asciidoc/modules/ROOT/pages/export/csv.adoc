...
Given this node: `CREATE (:Quote {name: 'foo,bar,baz', array:["a","b","c"], other: 123})`,
we can export without quotes:
...
Or else, using quotes only when is needed, 
in this case to wrap only `foo,bar,baz` and `["a","b","c"]`, in order to escape the separator character:
