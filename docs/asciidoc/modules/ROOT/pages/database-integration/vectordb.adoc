...
YIELD id, score, vector, metadata
----
...
[source,cypher]
----
CALL apoc.vectordb.chroma.get($host, '<collection_id>', ['1','2'], {<optional config>})
YIELD id, score, vector, metadata, text
----
...
CALL apoc.vectordb.weaviate.query($host, 
    ...
    5, 
    {fields: ["city", "foo"], <other optional config>})
YIELD id, score, vector, metadata
----
