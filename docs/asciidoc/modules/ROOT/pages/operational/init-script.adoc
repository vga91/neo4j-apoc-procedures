...
[NOTE]
====
You can also use `apoc.strict.initializer.<database_name>.<identifier>` that works as `apoc.initializer` but shuts down the db in case of errors.
For example: apoc.initializer.neo4j.0=return incorrectQuery
====
