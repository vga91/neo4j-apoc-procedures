...
The procedure accept 2 string argument, the `source` and the `dest`, representing 2 queries to compare,
and an optional `config` map as a 3rd parameter.

The procedure compares the `source` and `dest` two graphs and returns the differences in term of:
...
Please note that the node comparison leverage the existing constraint to find an equivalent node. To find a node using the internal id you can use `findById:true` config.
...
[source,cypher]
----
CALL apoc.diff.graphs("MATCH p = (start:Person)-[rel:KNOWS]->(end) RETURN start, rel, end", "MATCH p = (start)-[rel:KNOWS]->(end) RETURN start, rel, end", {dest: {target: {type: "DATABASE", value: "secondDb"}}})
----
...
Vice versa, in the `secondDb` database:
...
|===
| difference | 	entityType | 	id | 	sourceLabel | 	destLabel | 	source	 | dest
| "Destination Entity not found" | "Node" | 1 | "Person" | null	 | {"name": "Jerry Burton" } | null
| "Destination Entity not found" | "Node" | 2 | "Person" | null	 |{"name": "Jack William"}| null
| "Destination Entity not found" | 	"Relationship"	| 0	| "KNOWS" | 	null	 | {"start":{"name":"Jerry Burton"},"end":{"name":"Jack William"} "properties":{"time":"12:50:35.556000000+01:00","since":1999}} | null
|===
