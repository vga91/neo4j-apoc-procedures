...
| array | value of `array` config parameter (see table above) | indicates if field is an array
| arraySep | the value of `arraySep` config parameter (see table above) | separator for array
| name | none | rename field
| ignore | true if is included in `ignore` config parameter (see table above), otherwise false | ignore/remove this field
| nullValues | [] | which values to treat as null, e.g. `['na',false]`
|===
