...
With this data set (note that the 2 rels have different direction respect to `Movie` node):
[source,cypher]
----
...
we can use `includeDirections` to distinguish the direction with the suffix " >" and " <".

[source,cypher]
----
MATCH p1=(m:Movie {title:'M'})<-[:ACTED_IN {role:'R1'}]-(:Actor {name:'A1'}), 
p2 = (m)-[:ACTED_IN  {role:'R2'}]->(:Actor {name:'A2'})
WITH [p1,p2] as paths
CALL apoc.convert.toTree(paths, true, {includeDirections: true}) 
