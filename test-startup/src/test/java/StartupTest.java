...
public class StartupTest {
    ...
    private static File getProjectPath(String file, int endIndex, String full) {
        return Paths.get(file.substring(0, endIndex).concat(full)).toFile();
    }

    static {
        final String file = StartupTest.class.getClassLoader().getResource(".").getFile();
        final int endIndex = file.indexOf("test-startup");
        APOC_FULL = getProjectPath(file, endIndex, "full");
        APOC_CORE = getProjectPath(file, endIndex, "core");
    }
    ...
    private void startNeo4jContainerSession(Supplier<Neo4jContainerExtension> neo4jContainerCreation,
                                            Consumer<Session> sessionAction) {
