...
public class ComparePerformancesTest {
    ...
    public static void setUp() throws Exception {
        TestUtil.registerProcedure(db, ImportParquet.class, ExportParquet.class, ExportCSV.class, Meta.class, ImportCsv.class);
        apocConfig().setProperty(APOC_IMPORT_FILE_ENABLED, true);
        apocConfig().setProperty(APOC_EXPORT_FILE_ENABLED, true);

        IntStream.range(0, 50)
                .forEach(__-> db.executeTransactionally("UNWIND range(0, 9999) as id WITH id " +
    ...
    private void exportParquet() {
        testPerformanceCommon("CALL apoc.export.parquet.all('test.parquet')", "endParquet = ");
    }

    private void exportCsv() {
        testPerformanceCommon("CALL apoc.export.csv.all('test.csv', {bulkImport: true})", "endCsv = ");
    }
    ...
    private void progressInfoAssertion(Map<String, Object> r) {
        assertEquals(400000L, r.get("nodes"));
        assertEquals(200000L, r.get("relationships"));
    }
