...
public class CustomNewProcedureMultiDbTest {
    ...
    @Test
    public void testProceduresFunctionsInMultipleDatabase() throws Exception {
        ...
        String neo4jProc = "CALL custom.%s".formatted("neo4jProc");
        String neo4jFun = "RETURN custom.%s() AS answer".formatted("neo4jFun");
        assertEventually(() -> {
                    try {
                        long res = singleResultFirstColumn(neo4jSession, neo4jProc);
                        return 42L == res;
                    } catch (Exception e) {
                        return false;
                    }
        ...
        assertEventually(() -> {
                    try {
                        long res = singleResultFirstColumn(neo4jSession, neo4jFun);
                        return 42L == res;
                    } catch (Exception e) {
                        return false;
                    }
        ...
        try {
            testCall(neo4jSession, neo4jProc, r -> fail("Should fail due to not existing procedure "));
        } catch (Exception e) {
        ...
        try {
            testCall(neo4jSession, neo4jProc, r -> fail("Should fail due to not existing function"));
        } catch (Exception e) {
            String actualErrMsg = e.getMessage();
